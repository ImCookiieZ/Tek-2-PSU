...
void print_char_part64(elf64_s *elf, size_t idx, size_t shdr_nb)
{
    char tmp;
    size_t i;
...
void print_hex_part64(elf64_s *elf, size_t idx, size_t shdr_nb)
{
    unsigned char tmp;
...
void print_indentation64(elf64_s *elf, size_t idx, size_t shdr_nb)
{
    size_t i = idx;
...
void s_flag64(elf64_s *elf)
{
    for (size_t i = 0; i < elf->ehdr->e_shnum; i++)
    {
        ...
        for (size_t idx = 0; idx < elf->shdr[i].sh_size; idx += 16)
        {
            ...
            print_hex_part64(elf, idx, i);
            print_indentation64(elf, idx, i);
            printf("  ");
            print_char_part64(elf, idx, i);
            printf("\n");
        }
    }
}